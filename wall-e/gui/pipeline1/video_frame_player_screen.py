from tkinter import *
from gui.gui_base_frame import GuiBaseFrame
from gui.pipeline1.constants import VIDEO_HEIGHT, VIDEO_WIDTH
from gui.widgets.header1_label import Header1Label
from gui.widgets.p_label import PLabel
from utilities.image_converter import cv2_image_to_tkinter_with_resize

TITLE_ROW = 0
SUBTITLE_ROW = TITLE_ROW + 1
OFFSET_VALUES_ROW = SUBTITLE_ROW + 1
VIDEO_FRAMES_ROW = OFFSET_VALUES_ROW + 1
OFFSET_INC_INSTR_ROW = VIDEO_FRAMES_ROW + 1
OFFSET_INC_BUTTONS = OFFSET_INC_INSTR_ROW + 1
VIDEO_NAVIGATION_ROW = OFFSET_INC_BUTTONS + 1
VIDEO_BUTTONS_ROW_1 = VIDEO_NAVIGATION_ROW + 1
VIDEO_BUTTONS_ROW_2 = VIDEO_BUTTONS_ROW_1 + 1
NEXT_BUTTON_ROW = VIDEO_BUTTONS_ROW_2 + 1


class VideoFramePlayer(GuiBaseFrame):
    def __init__(self, parent, controller, **kw):
        GuiBaseFrame.__init__(self, parent, controller, **kw)
        self.left_video_filename = None
        self.right_video_filename = None
        self.video_frame_loader = None

    def setup_widgets(self):
        self.content_wrapper = Frame(self)

        self.title_label = Header1Label(self.content_wrapper, text="Frame Offset Validation")
        self.subtitle_label = PLabel(self.content_wrapper, text="Use the buttons below to ensure that the video frame offset is valid.")
        self.title_label.grid(row=TITLE_ROW, column=0, columnspan=6)
        self.subtitle_label.grid(row=SUBTITLE_ROW, column=0, columnspan=6)

        self.left_offset_label = PLabel(self.content_wrapper, text="tmp: left offset")
        self.right_offset_label = PLabel(self.content_wrapper, text="tmp: right offset")
        self.left_offset_label.grid(row=OFFSET_VALUES_ROW, column=0, columnspan=3)
        self.right_offset_label.grid(row=OFFSET_VALUES_ROW, column=3, columnspan=3)

        self.left_video_label = Label(self.content_wrapper)
        self.right_video_label = Label(self.content_wrapper)

        self.left_offset_inc_instr = PLabel(self.content_wrapper, text="Increase left video offset by:")
        self.right_offset_inc_instr = PLabel(self.content_wrapper, text="Increase right video offset by:")
        self.left_offset_inc_instr.grid(row=OFFSET_INC_INSTR_ROW, column=0, columnspan=3)
        self.right_offset_inc_instr.grid(row=OFFSET_INC_INSTR_ROW, column=3, columnspan=3)

        self.left_offset_inc_10_button = Button(self.content_wrapper, text="+10")
        self.left_offset_inc_5_button = Button(self.content_wrapper, text="+5")
        self.left_offset_inc_1_button = Button(self.content_wrapper, text="+1")
        self.right_offset_inc_1_button = Button(self.content_wrapper, text="+1")
        self.right_offset_inc_5_button = Button(self.content_wrapper, text="+5")
        self.right_offset_inc_10_button = Button(self.content_wrapper, text="+10")
        self.left_offset_inc_10_button.grid(row=OFFSET_INC_BUTTONS, column=0)
        self.left_offset_inc_5_button.grid(row=OFFSET_INC_BUTTONS, column=1)
        self.left_offset_inc_1_button.grid(row=OFFSET_INC_BUTTONS, column=2)
        self.right_offset_inc_1_button.grid(row=OFFSET_INC_BUTTONS, column=3)
        self.right_offset_inc_5_button.grid(row=OFFSET_INC_BUTTONS, column=4)
        self.right_offset_inc_10_button.grid(row=OFFSET_INC_BUTTONS, column=5)

        self.video_navigation_label = PLabel(self.content_wrapper, text="Video navigation:")
        self.video_navigation_label.grid(row=VIDEO_NAVIGATION_ROW, column=0, columnspan=6)

        self.left_video_frame_inc_10_button = Button(self.content_wrapper, text="<- 10")
        self.left_video_frame_inc_5_button = Button(self.content_wrapper, text="<- 5")
        self.left_video_frame_inc_1_button = Button(self.content_wrapper, text="<- 1")
        self.right_video_frame_inc_1_button = Button(self.content_wrapper, text="1 ->")
        self.right_video_frame_inc_5_button = Button(self.content_wrapper, text="5 ->")
        self.right_video_frame_inc_10_button = Button(self.content_wrapper, text="10 ->")
        self.left_video_frame_inc_10_button.grid(row=VIDEO_BUTTONS_ROW_1, column=0)
        self.left_video_frame_inc_5_button.grid(row=VIDEO_BUTTONS_ROW_1, column=1)
        self.left_video_frame_inc_1_button.grid(row=VIDEO_BUTTONS_ROW_1, column=2)
        self.right_video_frame_inc_1_button.grid(row=VIDEO_BUTTONS_ROW_1, column=3)
        self.right_video_frame_inc_5_button.grid(row=VIDEO_BUTTONS_ROW_1, column=4)
        self.right_video_frame_inc_10_button.grid(row=VIDEO_BUTTONS_ROW_1, column=5)

        self.left_video_frame_inc_300_button = Button(self.content_wrapper, text="<- 300")
        self.left_video_frame_inc_150_button = Button(self.content_wrapper, text="<- 150")
        self.left_video_frame_inc_30_button = Button(self.content_wrapper, text="<- 30")
        self.right_video_frame_inc_30_button = Button(self.content_wrapper, text="30 ->")
        self.right_video_frame_inc_150_button = Button(self.content_wrapper, text="150 ->")
        self.right_video_frame_inc_300_button = Button(self.content_wrapper, text="300 ->")
        self.left_video_frame_inc_300_button.grid(row=VIDEO_BUTTONS_ROW_2, column=0)
        self.left_video_frame_inc_150_button.grid(row=VIDEO_BUTTONS_ROW_2, column=1)
        self.left_video_frame_inc_30_button.grid(row=VIDEO_BUTTONS_ROW_2, column=2)
        self.right_video_frame_inc_30_button.grid(row=VIDEO_BUTTONS_ROW_2, column=3)
        self.right_video_frame_inc_150_button.grid(row=VIDEO_BUTTONS_ROW_2, column=4)
        self.right_video_frame_inc_300_button.grid(row=VIDEO_BUTTONS_ROW_2, column=5)

        self.next_button = Button(self.content_wrapper, text="Next")
        self.next_button.grid(row=NEXT_BUTTON_ROW, column=0, columnspan=6)

        self.content_wrapper.place(relx=0.5, rely=0.4, anchor=CENTER)

    def set_offset_values(self, left_offset=0, right_offset=0):
        self.left_offset = left_offset
        self.right_offset = right_offset

    def set_video_frame(self, frame_num):
        _, left_img = \
            self.controller.video_frame_loader.get_next_left_frame_tkinter_with_resize(frame_num + self.left_offset,
                                                                                       VIDEO_WIDTH, VIDEO_HEIGHT)
        _, right_img = \
            self.controller.video_frame_loader.get_next_right_frame_tkinter_with_resize(frame_num + self.right_offset,
                                                                                        VIDEO_WIDTH, VIDEO_HEIGHT)

        self.left_video_label.configure(image=left_img)
        self.left_video_label.image = left_img

        self.right_video_label.configure(image=right_img)
        self.right_video_label.image = right_img

    def start_video(self):
        # TODO Link frame matching offsets here
        self.set_video_frame(0)

    def start(self):
        # TODO
        pass

    def update_frame(self, data):
        pass

    def stop(self):
        # TODO
        pass
